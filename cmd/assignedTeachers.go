...
var assignedTeachersCmd = &cobra.Command{
	Use:   "assignedTeachers",
...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		getClasses(db, startRec, processAssignedTeachers)
	},
}

func processAssignedTeachers(classId int) {
	...
	stmt, err := db.Prepare(`INSERT INTO AssignedTeachers(ClassID,	SchoolID,	TeacherID, Role, ClassAlias)
	...
	for _, t := range row.Teachers[0:] {
		row := stmt.QueryRowContext(
			context.Background(),
			sql.Named("a", row.ClassID),
			sql.Named("b", Config.SchoolId),
			sql.Named("c", t.TeacherID),
			sql.Named("d", t.Role),
			sql.Named("e", t.ClassAlias))
...
func init() {
	...
	assignedTeachersCmd.Flags().StringP("class", "c", "", "class id to get sections enrollments for")

	assignedTeachersCmd.Flags().Int("startRec", 0, "starting class id to get sections enrollments")


	getCmd.AddCommand(assignedTeachersCmd)
